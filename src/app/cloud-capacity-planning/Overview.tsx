    ...
    <section className="space-y-4">
      ...
      <div className="flex gap-3 bg-sky-50 border border-sky-200 rounded-xl p-4">
        ...
        <p className="text-sm text-sky-900 leading-relaxed">
          ...
          model that treats each physical resource dimension (processors,
          RAM, storage, networking) as a separate capacity constraint. A VM
          call of class <InlineMath math="k" /> is accepted only when the
          system has enough free units of <em>every</em> resource it
          requires simultaneously. If any single dimension is full, the call
          is blocked, even if the remaining resources have spare capacity.
        </p>
      </div>
      ...
      <div className="flex gap-3 bg-violet-50 border border-violet-200 rounded-xl p-4">
        ...
        <p className="text-sm text-violet-900 leading-relaxed">
          ...
          <InlineMath math="\lambda_k" />. Each request needs a fixed amount
          of the four resources above, all at once. If there is enough free
          room in <em>all four</em> right now, the VM is created and stays
          running for some random amount of time (on average{" "}
          ...
          room, the request is rejected outright, it is not queued or
          retried, it is simply lost.
        </p>
      </div>
